...
public class VoxelChunk : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            transform.position = new Vector3(chunkPosition.x * chunkSize, 0.01f, chunkPosition.y * chunkSize);
        }
    ...
    void SetupMesh()
    {
        ...
        {
            Debug.LogWarning("쉐이더를 찾을 수 없습니다");
            vertexColorShader = Shader.Find("Unity/Color");
        }
    ...
    bool IsCaveAt(int x, int y, int z)
    {
        ...
        float cavel = Mathf.PerlinNoise(x * caveScale, z * caveScale);
        ...
        float caveValue = (cavel + cave2 + cave3) / 3;

        return caveValue > caveThreshold;
    }

    BlockType GetStoneWithOre(int x, int z, int y)
    {
    ...
    int GetTerrainHeight(int worldX, int worldZ)
    {
        ...
        for(int i =0;i<octaves;i++)
        {
            ...
            noiseHeight += perlinValue * amplitube;

            amplitube *= persistence;
            frequency *= persistence;
        }

        int height = groundLevel + Mathf.RoundToInt(noiseHeight * heighVariation);
    ...
    bool IsTranparent(int x, int y , int z)
    {
    ...
    void AddBlockFaces(int x, int y, int z, BlockType block, List<Vector3> vertices, List<int > triagles, List<Color> colors)
    {
        BlockData blockData = new BlockData(block);

        if(IsTranparent(x, y+1, z))
        {
            AddFace(x, y, z, Vector3.up, blockData.blockColor, vertices, triagles, colors);
        }
        if (IsTranparent(x, y - 1, z))
        {
            AddFace(x, y, z, Vector3.down, blockData.blockColor, vertices, triagles, colors);
        }
        if (IsTranparent(x, y, z+1))
        {
            AddFace(x, y, z, Vector3.forward, blockData.blockColor, vertices, triagles, colors);
        }
        if (IsTranparent(x, y, z-1))
        {
            AddFace(x, y, z, Vector3.back, blockData.blockColor, vertices, triagles, colors);
        }
        if (IsTranparent(x+1, y, z))
        {
            AddFace(x, y, z, Vector3.right, blockData.blockColor, vertices, triagles, colors);
        }
        if (IsTranparent(x-1, y, z))
        {
            AddFace(x, y, z, Vector3.left, blockData.blockColor, vertices, triagles, colors);
        }
    }
        ...
        for(int x = 0; x < chunkSize; x++)
        {
            for(int y = 0; y < chunkHeight; y++)
            {
                for(int z = 0; z < chunkSize; z++)
                {
                    ...
                    AddBlockFaces(x, y, z, block, vertices, triagles, colors);
                }
            }
        }
